...
all: unit-tests lint tasker tasker-image
...
fmt:  ## Formats the source code
	@nerdctl run --rm -it -v $(PWD):/src -w /src $(TOOLCHAIN) \
		...
		gofumpt -w . && \
        goimports -w -local github.com/sentinelos/tasker ."
...
.PHONY: tasker-linux-amd64
tasker-linux-amd64:  ## Builds executables for tasker platform linux/amd64.
	@$(MAKE) tasker-linux-amd64-local PLATFORM_ARGS="--platform=linux/amd64"

.PHONY: tasker-linux-arm64
tasker-linux-arm64:  ## Builds executables for tasker platform linux/arm64.
	@$(MAKE) tasker-linux-arm64-local PLATFORM_ARGS="--platform=linux/arm64"

.PHONY: tasker
tasker: tasker-linux-amd64 tasker-linux-arm64  ## Builds executables for tasker.
...
.PHONY: tasker-image
tasker-image:  ## Builds image for tasker.
